...
module `MODNAME_TXN #(
  ...
  x_read_outstanding_bound: `TXN_SOURCE property (
    ar_hsk |-> rd_outstanding < MAX_READ_OUTSTANDING || r_complete);
  x_write_outstanding_bound: `TXN_SOURCE property (
  ...
  always_ff @(posedge clk or negedge rstn) begin
    if (!rstn) begin
      ...
      endcase
      case ({aw_hsk, b_hsk})
        2'b10: wr_outstanding <= wr_outstanding + 1'b1;
  ...
  `MODNAME_PAIR_TRACKER #(
    ...
    .clk(clk), .rstn(rstn),
    .aw_hsk(aw_hsk), .aw_addr(axi.aw_addr), .aw_len(axi.aw_len),
