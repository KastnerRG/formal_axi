...
//   1: external Manager, DUT Subordinate (manager_axi_fvip)
//   0: DUT Manager, external Subordinate (subordinate_axi_fvip)
//
...
`define SUBORDINATE_RULE(NAME, EXPR)                                \
...
module axi_channel_fvip #(
  ...
  generate
    ...
    `MANAGER_RULE(a_manager_valid_low_after_reset,
      @(posedge clk) disable iff ($isunknown(rstn))
        !rstn |=> !(axi.aw_valid || axi.w_valid || axi.ar_valid))
    `SUBORDINATE_RULE(a_subordinate_valid_low_after_reset,
      @(posedge clk) disable iff ($isunknown(rstn))
    ...
    `MANAGER_RULE(a_w_payload_stall_stable,
      axi.w_valid && !axi.w_ready |=>
        $stable({axi.w_data, axi.w_strb, axi.w_last, axi.w_user}))
    `MANAGER_RULE(a_ar_payload_stall_stable,
    ...
    `MANAGER_RULE(a_aw_cache_non_mod,
      axi.aw_valid && !axi.aw_cache[1] |-> axi.aw_cache[3:2] == 2'b00)
    `MANAGER_RULE(a_ar_cache_non_mod,
    ...
    if (ENABLE_EXCLUSIVE) begin : g_exclusive
      `MANAGER_RULE(a_aw_excl_addr_aligned, axi.aw_valid && axi.aw_lock |-> (axi.aw_addr & (total_bytes(axi.aw_len, axi.aw_size) - 1)) == '0)
      `MANAGER_RULE(a_ar_excl_addr_aligned, axi.ar_valid && axi.ar_lock |-> (axi.ar_addr & (total_bytes(axi.ar_len, axi.ar_size) - 1)) == '0)
      `MANAGER_RULE(a_aw_excl_max_bytes, axi.aw_valid && axi.aw_lock |-> total_bytes(axi.aw_len, axi.aw_size) <= 128)
      `MANAGER_RULE(a_ar_excl_max_bytes, axi.ar_valid && axi.ar_lock |-> total_bytes(axi.ar_len, axi.ar_size) <= 128)
      `MANAGER_RULE(a_aw_excl_len, axi.aw_valid && axi.aw_lock |-> axi.aw_len <= 8'd15)
      `MANAGER_RULE(a_ar_excl_len, axi.ar_valid && axi.ar_lock |-> axi.ar_len <= 8'd15)
      `MANAGER_RULE(a_aw_excl_bytes_pow2, axi.aw_valid && axi.aw_lock |-> $onehot(total_bytes(axi.aw_len, axi.aw_size)))
      `MANAGER_RULE(a_ar_excl_bytes_pow2, axi.ar_valid && axi.ar_lock |-> $onehot(total_bytes(axi.ar_len, axi.ar_size)))
      `MANAGER_RULE(a_aw_excl_cache, axi.aw_valid && axi.aw_lock |-> axi.aw_cache[3:2] == 2'b00)
      `MANAGER_RULE(a_ar_excl_cache, axi.ar_valid && axi.ar_lock |-> axi.ar_cache[3:2] == 2'b00)
      ...
      `MANAGER_RULE(c_profile_aw_lock_disabled,
        axi.aw_valid |-> !axi.aw_lock)
      `MANAGER_RULE(c_profile_ar_lock_disabled,
    ...
    if (!ENABLE_EXCLUSIVE) begin : g_no_exokay
      `SUBORDINATE_RULE(a_b_no_exokay,
        axi.b_valid |-> axi.b_resp != axi_pkg::RESP_EXOKAY)
      `SUBORDINATE_RULE(a_r_no_exokay,
        axi.r_valid |-> axi.r_resp != axi_pkg::RESP_EXOKAY)
    end

    // Remaining intra-channel profile bounds and optional progress policies.
    `MANAGER_RULE(c_profile_aw_burst_len,
      axi.aw_valid |-> axi.aw_len < MAX_BURST_LEN)
    `MANAGER_RULE(c_profile_ar_burst_len,
