...
interface axi_fvip_txn_view_if #(
  ...
  logic [STATE_W-1:0] wr_rank;              // Same-ID B responses ahead of the chosen write.
  ...
  modport Producer (
    output req, rsp,
    output rd_select, rd_selected, rd_pending, rd_completed,
    ...
    output wr_select, wr_selected, wr_pending, wr_completed,
      wr_rank, wr_outstanding, wr_aw, wr_data_complete, wr_b,
      wr_rsp_visible, wr_rsp_complete, wr_rsp_wait_cycles,
    output pair_select_aw, pair_select_w, pair_selected, pair_pending_aw,
      pair_pending_w, pair_completed, pair_rank, pair_aw, wr_beat_idx,
      wr_w, wr_beat_sampled, wr_data_visible,
      wr_data_burst_complete, wr_data_wait_cycles
  );

  modport Consumer (
    input req, rsp,
    input rd_select, rd_selected, rd_pending, rd_completed,
      rd_rank, rd_outstanding, rd_ar, rd_beat_idx,
      rd_rsp_beat, rd_r, rd_beat_sampled,
      rd_rsp_visible, rd_rsp_complete, rd_rsp_wait_cycles,
    input wr_select, wr_selected, wr_pending, wr_completed,
      wr_rank, wr_outstanding, wr_aw, wr_data_complete, wr_b,
      wr_rsp_visible, wr_rsp_complete, wr_rsp_wait_cycles,
